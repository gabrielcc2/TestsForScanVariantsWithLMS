...
class ScanVariantsTests extends TutorialFunSuite {
  ...
  test("ScanVariants") {
   ...
   val snippet = new DslDriverC[Array[Float],Unit] {
	...
	Takes as input an array of floats. By convention, the first item is the value for comparison, the second item is the size of  		the input array or number of tuples, the  third the number of threads. The next 2*number of threads spaces are used by the 		generator. The following number of tuples items are the input, and the last number of tuples items are where the output will 		be stored. 
	In this way we pack both the input and the needed memory to be allocated. It is generated in the preceeding part of the code, 		which is defined in the dsl api ...**/
	...
	def snippet(input: Rep[Array[Float]]) = comment("Scan Variants- timer goes here", verbose = true) {
		...
		/*For ease of work, and after observing some unexpected typing issues (and forward references problems), we decided 			to place here the definition of the loop classes, in charge of handling the application of the variants.*/
   ...
   /*Post-processing of the generated file so as to include Vectorization and Parallelization functions*/   
