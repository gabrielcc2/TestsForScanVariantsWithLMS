...
trait DslGenC extends CGenNumericOps
  ...
  override def isPrimitiveType(tpe: String) : Boolean = {
    tpe match {
      case "char*" => true
      case "char" => true
      case "float*" => true //This was not in the original code, it was a hack to avoid  double asteriscs.
      case "int*" => true //This was not in the original code, it was a hack to avoid  double asteriscs.
...
abstract class DslDriverC[A:Manifest,B:Manifest] extends DslSnippet[A,B] with DslExp { q =>
  ...
  lazy val code: String = {
    val source = new java.io.StringWriter()
    codegen.emitSource(snippet, "Scan", new java.io.PrintWriter(source))
    source.toString
  }
